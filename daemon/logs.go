...
func (daemon *Daemon) ContainerLogs(name string, config *ContainerLogsConfig) error {
	...
	if container.LogDriverType() != "json-file" &&
		!(container.LogDriverType() == "stream" && config.Follow && container.IsRunning()) {

		return fmt.Errorf("\"logs\" endpoint is supported only for \"json-file\" logging driver, or in follow mode for running containers for the \"stream\" driver")
	}
	if container.LogDriverType() == "json-file" {
	...
	if container.LogDriverType() == "stream" && config.Follow && container.IsRunning() {

		stream := container.logDriver.(*stream.Stream)
		c := stream.Output()
		for msg := range c {
